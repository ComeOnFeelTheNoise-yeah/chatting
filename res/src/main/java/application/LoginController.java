   ...
   User user;
    ...
    private boolean isValidation() {
       ...
       Pattern pattern = Pattern.compile("^[\\w-]+(\\.[\\w-]+)*@[\\w-]+(\\.[\\w-]+)*(\\.[a-zA-Z]{2,})$");
        ...
        if(!matcherPw.find()) {
           errorLabel.setText("비밀번호 형식이 유효하지 않습니다! \n비밀번호는 소문자 알파벳, 숫자, 특수문자 !?@#$%^&+= 를 포함해야하며 6~16자로 구성되어야합니다 ㅠ");  // 유효하지 않으면 에러 메시지 표시
           errorLabel.setStyle("-fx-text-fill: red;");
           pwInput.requestFocus();
           return false;
        }
    ...
    protected void goLogin() {
        if (isValidation()) {
            ...
            ref.orderByChild("email").equalTo(idInput.getText()).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
